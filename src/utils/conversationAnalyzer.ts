...
export class ConversationAnalyzer {
  ...
  private static generateSummary(score: number, issues: Issue[]): string {
    if (score >= 80) {
      return "This conversation went relatively well with only minor areas for improvement.";
    } else if (score >= 60) {
      return "The conversation had some challenges, but with adjustments, future interactions could improve significantly.";
    } else {
      const mainIssue = issues.find(issue => issue.severity === 'high');
      if (mainIssue) {
        return `The conversation struggled primarily due to ${mainIssue.category} issues, but these are learnable skills that can be improved.`;
      }
      return "This conversation had several challenges that contributed to the outcome, but these are all areas that can be improved with practice.";
    }
  }
